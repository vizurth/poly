...
\textbf{Сложность:} $O(V^3)$ по времени, $O(V)$ по памяти.
...
\begin{lstlisting}[style=cstyle, caption={Поиск всех маршрутов (graph.cpp)}]
template <typename T>
...
\textbf{Сложность:} по времени $O(V!)$, по памяти $O(V)$. 