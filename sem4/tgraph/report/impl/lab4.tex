...
Затем берётся алгеброическое дополнение $B'$ к элементу $B[0][0]$~-- это матрица размера $(n-1) \times (n-1)$,
без нулевой строки и нулевого столбца.  Определитель считается алгоритмом Барейсса, который сохраняет целочисленность  значений.
...
остову: если между концами ребра уже есть путь~-- значит добавление этого ребра создаёт цикл, и его пропускают.
...
\texttt{isIndependent(S, v)} проверяет, зависима ли вершина $v$ от множества $S$, перебирая все вершины $u \in S$ и проверяя смежность.