...
\begin{lstlisting}[style=cstyle, caption={Алгоритм Барейсса(kirchhoff.cpp)}]
for (int k = 0; k < m; k++) {
    int pivot = -1;
    for (int i = k; i < m; i++)
...
\textbf{Сложность:} $O(V^4)$ по времени;
$O(V)$ по памяти.
...
\begin{lstlisting}[style=cstyle, caption={Исходный код (mis.cpp)}]
