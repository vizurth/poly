...
Сложность: $O(V(V + E))$ при матричном представлении, где один BFS
стоит $O(V^2)$.
...
\textbf{Выход:} матрица кратчайших и матрица наидлиннейших путей
...
Временная сложность: $O(k \cdot n^3)$, пространственная: $O(n^2)$.
...
Когда доходим до \texttt{target}~--- сохраняем текущий путь \texttt{currentPath}.
При выходе из рекурсии вершина снимается с отметки (\textit{backtracking}):
