...
Shimbel::Shimbel(const Graph& g) : graph(g), numVertices(g.getNumVertices()) {}
...
std::vector<std::vector<double>> Shimbel::findMinPaths(int k) {
    // инициализация: D[0] - нулевая матрица (путь длины 0 от вершины к себе)
    std::vector<std::vector<double>> prev(numVertices, std::vector<double>(numVertices, INF));
    ...
    std::vector<std::vector<double>> current(numVertices, std::vector<double>(numVertices, INF));
    for (int i = 0; i < numVertices; i++) {
    ...
    for (int m = 2; m <= k; m++) {
        std::vector<std::vector<double>> next(numVertices, std::vector<double>(numVertices, INF));
        
        for (int i = 0; i < numVertices; i++) {
            for (int j = 0; j < numVertices; j++) {
                for (int v = 0; v < numVertices; v++) {
                    if (current[i][v] != INF && adjMatrix[v][j] > 0.0) {
...
std::vector<std::vector<double>> Shimbel::findMaxPaths(int k) {
...
void Shimbel::printPathMatrix(const std::vector<std::vector<double>>& matrix, 
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < n; j++) {
            if (std::isinf(matrix[i][j]) || matrix[i][j] == -std::numeric_limits<double>::infinity()) {
