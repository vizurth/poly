...
При запуске программа выводит приветственное сообщение и информацию о командах.  Пользователь может вводить команды или арифметические выражения. (см. рисунок \ref{fig:startup})
...
При вводе команды \texttt{help} программа выводит список всех доступных команд и их описание. (см. рисунок \ref{fig:help})
...
При вводе команды \texttt{info} программа выводит информацию о конфигурации системы. (см. рисунок \ref{fig:info})
...
При вводе команды \texttt{hasse} программа выводит диаграмму Хассе - визуальное представление отношения порядка на алфавите. (см. рисунок \ref{fig:hasse})
...
\subsection{Сценарий 3: Действия с большими числами}

\subsubsection{Сложение положительных чисел}

Пользователь вводит выражение для сложения двух положительных чисел. (см. рисунок \ref{fig:add_positive})
...
Пример, демонстрирующий механизм переноса в старшие разряды. (см. рисунок \ref{fig:add_carry})
...
При сложении \texttt{hhhh + b} происходит переход через границу алфавита, что вызывает перенос в следующий разряд.

\subsubsection{Сложение с отрицательными числами}

Пример сложения чисел с разными знаками. (см. рисунок \ref{fig:add_negative})
...
При вычитании меньшего из большего результат положительный, при вычитании большего из меньшего - отрицательный. (см. рисунок \ref{fig:subtract_positive})

\subsubsection{Вычитание с заимствованием}

Пример, демонстрирующий механизм заимствования из старших разрядов. (см. рисунок \ref{fig:subtract_borrow})
...
При вычитании \texttt{baaa - hhh} происходит заимствование из старшего разряда.
...
Умножение выполняется столбиком с промежуточным суммированием. (см. рисунок \ref{fig:multiply_positive})
...
Знак результата определяется правилом (см. рисунок \ref{fig:multiply_negative}): 
\begin{itemize}
    \item $+ \times + = +$
    \item $- \times - = +$
    \item $+ \times - = -$
...
При вводе числа, содержащего символы вне алфавита, программа выводит сообщение об ошибке. (см. рисунок \ref{fig:error_format})
...
При попытке деления на нейтральный элемент по сложению программа выводит ошибку. (см. рисунок \ref{fig:error_divide_zero})
...
При превышении максимального количества разрядов (MAX\_DIGITS = 8) программа выводит ошибку переполнения. (см. рисунок \ref{fig:error_overflow})
...
Результат действия превышает 8 разрядов, что не допускается системой.

\subsubsection{Неизвестная команда}

При вводе неизвестной команды программа выводит сообщение об ошибке. (MAX\_DIGITS = 8) программа выводит ошибку переполнения. (см. рисунок \ref{fig:error_unknown})
...
При вводе команды \texttt{exit} или \texttt{quit} программа завершает работу с прощальным сообщением. (см. рисунок \ref{fig:exit})
