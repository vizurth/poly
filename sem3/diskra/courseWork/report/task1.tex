...
\begin{itemize}
    ...
    \item \texttt{BigArithmeticCalc.hpp} и \texttt{BigArithmeticCalc.cpp}: содержат реализацию большой конечной арифметики. Класс выполняет действия столбиком над многозначными числами.
...
additionTableWithCarry}: таблица сложения с переносом.
...
Класс \texttt{SmallArithmetic} реализует малую конечную арифметику на заданном алфавите с правилом "$+1$". Все действия выполняются через построение диаграммы Хассе.
...
\textbf{Описание:} Возвращает следующий элемент алфавита согласно правилу "$+1$".  Используется для обхода диаграммы Хассе при выполнении операций сложения и умножения. 
...
\textbf{Описание:} Сравнивает два элемента алфавита, определяя их относительный порядок в диаграмме Хассе.
...
\textbf{Выход:}
\begin{itemize}
    \item \texttt{int}: $0$ (если $a = b$), $-1$ (если $a < b$), $1$ (если $a > b$) флаг сравнения.
\end{itemize}
...
\textbf{Описание:} Выполняет сложение двух элементов алфавита путём последовательного применения правила "$+1$".  \\ Результат вычисляется как $a + b = a \, \underbrace{+ \,1 +1 \ldots +1}_{b \text{ раз}}$. 
...
\begin{enumerate}
    ...
    \item Инициализируется счётчик значением нейтрального элемента;
...
\textbf{Описание:} Выполняет умножение двух элементов алфавита через повторное сложение. Результат вычисляется как $a \times b = \underbrace{a + a + \ldots + a}_{b \text{ раз}}$. 
...
Алгоритм: 
\begin{enumerate}
    \item Если имеем произведение двух нейтральных элементов по сложению получаем универсум;
...
\textbf{Описание:} Выполняет вычитание путём поиска такого элемента $c$, что $b + c = a$.
...
\textbf{Описание:} Строит таблицу для сложения с учётом входящего переноса. Таблица используется в большой арифметике при сложении столбиком.  Для каждой тройки $(a, b, c_{in})$ вычисляется пара $(sum, c_{out})$.

\textbf{Вход:}
\begin{itemize}
    \item \texttt{vector<string>} alphabet
\end{itemize}

\textbf{Выход:}
\begin{itemize}
    \item \texttt{map<tuple<string, string, string>, pair<string, string>> additionTableWithCarry} - таблица для сложения с учётом входящего переноса.
\end{itemize}
...
Класс \texttt{BigArithmeticCalc} реализует большую конечную арифметику - действия над многозначными числами. Внутри использует класс \texttt{SmallArithmetic} для посимвольных операций и действий.
...
% \textbf{Описание:} Проверяет, является ли число отрицательным (начинается ли строка с символа минус).
...
\textbf{Описание:} Удаляет знак минус из начала строки, возвращая абсолютное значение числа.

\textbf{Вход:}
\begin{itemize}
    \item \texttt{const string\& num}: Число.
\end{itemize}
...
\textbf{Описание:} Добавляет знак минус к числу, если указан флаг отрицательности.  Нейтральный элемент по сложению всегда возвращается без знака. 

\textbf{Вход:}
\begin{itemize}
    \item \texttt{const string\& num}:  Число без знака.
    \item \texttt{bool}: Флаг отрицательности. 
\end{itemize}
...
% \textbf{Описание:} Проверяет, что все символы числа принадлежат алфавиту системы.
...
% \textbf{Описание:} Проверяет, не превышает ли количество разрядов числа максимально допустимое значение.
...
\textbf{Описание:} Удаляет ведущие нули из числа.  Если число состоит только из нулей, возвращает нейтральный элемент по сложению.
...
\textbf{Описание:} Сравнивает два беззнаковых многозначных числа.
...
\subsection*{Методы реализации действий большой арифметики}

\subsubsection{Метод addBigUnsigned(const string\& a, const string\& b)}

\textbf{Описание:} Выполняет сложение двух беззнаковых многозначных чисел столбиком с учётом переноса между разрядами.
...
\item Проверка валидности входных строк;
\item Выравнивание чисел по длине путём добавления ведущих нулей к более короткому;
\item Поразрядное сложение справа налево с учётом переноса;
\item Добавление финального переноса в старший разряд при его наличии;
\item Проверка результата на переполнение;
\item Удаление ведущих нулей из результата.
\end{enumerate}

\subsubsection{Метод subtractBigUnsigned(const string\& a, const string\& b)}

\textbf{Описание:} Выполняет вычитание двух беззнаковых многозначных чисел столбиком с учётом заимствования из старших разрядов.  Требует, чтобы $a \geq b$.
...
\item Поразрядное вычитание справа налево с учётом заема;
...
\textbf{Описание:} Умножает многозначное число на однозначное число (один элемент алфавита). В процессе работы последовательно формируются неполные произведения путём многократного суммирования исходного числа. Для определения количества операций сложения используется буквенный счётчик, который увеличивается до тех пор, пока не достигнет значения множителя.
...
\textbf{Описание:} Выполняет умножение двух беззнаковых многозначных чисел столбиком.  Число $a$ умножается на каждую цифру числа $b$, неполные произведения сдвигаются и суммируются.
...
\begin{enumerate}
    ...
    \item Сдвиг неполные произведений влево на соответствующее количество разрядов;
    \item Суммирование всех неполные произведения. 
    \item Удаление ведущих нулей из результата.
\end{enumerate}
...
\textbf{Описание:} Выполняет деление двух беззнаковых многозначных чисел "уголком".  Возвращает частное и остаток.

\textbf{Вход: }
\begin{itemize}
    \item \texttt{const string\& a}: Делимое.
    \item \texttt{const string\& b}: Делитель.
\end{itemize}

\textbf{Выход:}
\begin{itemize}
    \item \texttt{pair<string, string>}: частное, остаток.
\end{itemize}
...
Алгоритм:
\begin{enumerate}
\item Проверка корректности входных данных и обработка cлучаев: деление на ноль, нулевое делимое;
\item Сравнение делимого и делителя для раннего завершения, если делимое меньше делителя;
\item Последовательный просмотр цифр делимого слева направо с накоплением текущего остатка;
\item Для каждого шага подбор максимальной цифры частного, произведение которой на делитель не превышает текущий остаток;
\item Добавление найденной цифры в частное и вычитание соответствующего произведения из остатка;
\item Удаление ведущих нулей из частного и остатка.
\end{enumerate}

\subsection*{Публичные действия с учётом знаков}

\subsubsection{Метод add(const string\& a, const string\& b)}

\textbf{Описание: } Выполняет сложение двух чисел с учётом знаков. Обрабатывает случаи сложения чисел с одинаковыми и разными знаками.
...
\textbf{Описание:} Выполняет вычитание двух чисел с учётом знаков. Обрабатывает случаи вычитания чисел с одинаковыми и разными знаками.
...
\textbf{Описание:} Выполняет умножение двух чисел с учётом знаков.  Знак результата определяется правилом: $(-) \times (-) = (+)$, $(+) \times (-) = (-)$, $(+) \times (+) = (+)$.
...
\textbf{Описание: } Выполняет деление двух чисел с учётом знаков. Возвращает частное и остаток.
...
Алгоритм:
\begin{enumerate}
\item Выполнение деления беззнаковых чисел с получением частного и остатка, проверка деления на ноль происходит внутри метода \texttt{divideBigUnsigned}.

\item Обработка специальных состояний (пустое множество, универсум, ошибка)

\item Обработка комбинаций знаков:
\begin{itemize}
    \item Если делимое отрицательное, а делитель положительный и $r \ne 0$:
    \[
    -a / b = -(a / b + 1)
    \]
    Остаток пересчитывается $r = b - r$
    
    \item Если делимое положительное, а делитель отрицательный:
    \[
    a / (-b) = -(a/b)
    \]
    Остаток сохраняется.
    
    \item Если оба числа отрицательные:
    \[
    -a / (-b) = a/b
    \]
    Остаток сохраняется.
...
\item Формирование итоговой строки результата в виде частного и остатка с корректным знаком частного.
