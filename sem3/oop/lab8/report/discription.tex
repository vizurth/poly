...
\begin{itemize}
    \item нормализация входной строки имени и проверка её на пустое значение;
    \item удаление начальных и конечных пробелов у исходного ввода;
    \item проверка того, что имя не начинается и не заканчивается дефисом;
    \item проверка допустимого набора символов (буквы латинского и кириллического алфавита, цифры, пробел и дефис);
    \item проверка отсутствия двойных дефисов и последовательностей из нескольких пробелов.
\end{itemize}

В случае обнаружения ошибки метод возвращает значение \texttt{false} и записывает описание причины ошибки в параметр \texttt{errorMessage}.  
При успешном прохождении всех проверок метод возвращает значение \texttt{true}.
...
\subsubsection*{Метод load():} \\
Метод \texttt{load} выполняет загрузку списка контактов из файла в формате JSON.

Основные этапы работы метода:
\begin{itemize}
    \item проверка существования файла с данными контактов; при отсутствии файла список контактов очищается и метод завершается успешно;
    \item открытие файла в режиме чтения и загрузка его содержимого в память;
    \item разбор JSON-данных и обработка возможных ошибок парсинга;
    \item проверка того, что корневой элемент JSON-документа является массивом;
    \item очистка текущего списка контактов;
    \item последовательное преобразование элементов JSON-массива в объекты класса \texttt{Contact} и добавление их в коллекцию контактов.
\end{itemize}

В случае возникновения ошибок при открытии файла или разборе JSON метод возвращает значение \texttt{false}.  
При успешной загрузке данных метод возвращает значение \texttt{true}.

\subsubsection*{Метод save():}

Метод \texttt{save} выполняет сохранение списка контактов в файл в формате JSON.

Основные этапы работы метода:
\begin{itemize}
    \item открытие файла для записи по указанному имени;
    \item формирование JSON-массива на основе текущего списка контактов;
    \item преобразование объектов класса \texttt{Contact} в формат JSON и добавление их в массив;
    \item создание JSON-документа и запись его в файл;
    \item закрытие файла после завершения записи данных.
\end{itemize}

В случае невозможности открыть файл для записи метод возвращает значение \texttt{false}.  
При успешном сохранении данных метод возвращает значение \texttt{true}.
...
Процесс добавления нового или обновления существующего контакта включает несколько этапов проверки и обработки данных. Блок-схема представлена на рис. \ref{fig:addContact}.

Основные этапы работы метода \texttt{addContact}:
\begin{itemize}
    \item проверка корректности заполнения формы методом \texttt{validateForm}; при обнаружении ошибок пользователю выводится сообщение, выполнение метода прекращается;
    \item проверка предупреждений о форматировании данных (например, нестандартное оформление ФИО или номеров телефонов) с возможностью подтверждения продолжения пользователем; при отказе выполнение метода прекращается;
    \item создание объекта \texttt{Contact} на основе данных формы;
    \item проверка на наличие дубликатов по ключевым полям (ФИО и Email); при обнаружении дубликата пользователю выводится сообщение об ошибке, выполнение метода прекращается;
    \item если выполняется редактирование существующего контакта:
    \begin{itemize}
        \item сохранение старого состояния контакта для возможности отмены изменений;
        \item обновление контакта в хранилище;
        \item вывод пользователю сообщения об успешном обновлении.
    \end{itemize}
    \item если добавляется новый контакт:
    \begin{itemize}
        \item добавление контакта в хранилище;
        \item фиксация действия для истории изменений;
        \item вывод пользователю сообщения об успешном добавлении.
    \end{itemize}
    \item завершение обработки:
    \begin{itemize}
        \item очистка формы ввода;
        \item обновление таблицы контактов;
        \item сохранение данных в файл;
        \item сброс флагов редактирования и управление состоянием кнопок интерфейса.
    \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{myPhoto/addContext.png}
    \caption{Блок-схема метода addContact()}
    \label{fig:addContact}
\end{figure}


\subsubsection*{Метода editContact():}

Процесс редактирования существующего контакта включает несколько этапов проверки и подготовки данных. Блок-схема представлена на рис. \ref{fig:editContact}.

Основные этапы работы метода \texttt{editContact}:
\begin{itemize}
    \item получение выбранной строки из таблицы контактов;
    \item проверка, что контакт выбран; при отсутствии выбора выводится сообщение об ошибке, выполнение метода прекращается;
    \item извлечение данных выбранного контакта (Фамилия, Имя, Email) из таблицы;
    \item поиск контакта в хранилище по ключевым полям (Фамилия, Имя, Email); если контакт не найден, выводится сообщение об ошибке и метод завершает работу;
    \item установка индекса редактируемого контакта и флага редактирования;
    \item заполнение формы данными выбранного контакта для последующего редактирования;
    \item изменение состояния элементов интерфейса: блокировка кнопок добавления, редактирования и удаления, активация кнопок сохранения и отмены.
\end{itemize}


\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{myPhoto/EditContact.png}
    \caption{Блок-схема метода editContact()}
    \label{fig:editContact}
\end{figure}
...
\subsubsection*{Метод searchContacts():}
Процесс поиска контактов в таблице включает фильтрацию и отображение результатов. Блок-схема представлена на рис. \ref{fig:searchContacts}.

Основные этапы работы метода \texttt{searchContacts}:
\begin{itemize}
    \item определение выбранного поля поиска;
    \item если выбран поиск по дате рождения:
    \begin{itemize}
        \item получение даты и типа поиска (точная дата, год или месяц и год);
        \item фильтрация контактов по дате;
        \item обновление таблицы с найденными контактами.
    \end{itemize}
    \item если выбран текстовый поиск:
    \begin{itemize}
        \item получение текста поиска;
        \item при пустом поисковом запросе таблица заполняется всеми контактами;
        \item фильтрация контактов по выбранному полю или по всем полям;
        \item обновление таблицы с найденными контактами.
    \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{myPhoto/searchContacts.png}
    \caption{Блок-схема метода searchContacts()}
    \label{fig:searchContacts}
\end{figure}
\newpage
...
\subsubsection*{Метод undoLastAction:}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.15\linewidth]{myPhoto/undoLastAction.png}
    \caption{Блок-схема метода undoLastAction()}
...
При первом запуске приложения отображается главное окно с пустой таблицей контактов и формой ввода данных (см. рисунок~\ref{fig:first_start}). Если файл \texttt{phonebook.json} не существует, он будет автоматически создан при сохранении первого контакта.
...
Пользователь заполняет все поля формы корректными данными (см. рисунок~\ref{fig:add_form_filled}):
...
\begin{enumerate}
    ...
    \item Показ информационного сообщения об успешном добавлении (см. рисунок~\ref{fig:contact_added}).
\end{enumerate}
...
Если длина телефона меньше 10 цифр, выводится ошибка: "Телефон должен содержать минимум 10 цифр" (см. рисунок~\ref{fig:add_table}).
...
После нажатия кнопки "Редактировать" (см. рисунок~\ref{fig:edit_mode}):
...
\begin{enumerate}
    ...
    \item Показ сообщения об успешном редактировании(см. рисунок~\ref{fig:contact_edited}).
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Показ сообщения "Контакт удалён" (см. рисунок~\ref{fig:contact_deleted}).
\end{enumerate}
...
При выборе поля "Дата рождения" (см. рисунок~\ref{fig:date_search}):
...
Пользователь выбирает дату "08.01.2006" и тип "Точное совпадение". В таблице отображаются только контакты, родившиеся именно в этот день (см. рисунок~\ref{fig:date_search_result_1}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{myPhoto/current_date.png}
    \caption{Результат поиска по дате (08.01.2006)}
    \label{fig:date_search_result_1}
\end{figure}

\textbf{Пример 2: Год}

Пользователь выбирает дату "08.01.2006" и тип "Год. В таблице отображаются все контакты, родившиеся в 2006 году (см. рисунок~\ref{fig:date_search_result_2}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{myPhoto/date_year_search.png}
    \caption{Результат поиска по году рождения (2006)}
    \label{fig:date_search_result_2}
\end{figure}

\textbf{Пример 3: Месяц и год}

Пользователь выбирает дату "08.12.2006" и тип "Месяц и год". В таблице отображаются все контакты, родившиеся в декабре 2006 года (см. рисунок~\ref{fig:date_search_result_3}).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{myPhoto/date_year_month.png}
    \caption{Результат поиска по месяцу и году (декабрь 2005)}
    \label{fig:date_search_result_3}
\end{figure}
...
\subsection{Сценарий 6: Отмена последнего действия}

\subsubsection{Отмена добавления}

Пользователь добавил контакт, но сразу нажал кнопку "Отменить". Происходит (см. рисунок~\ref{fig:undo_action} и ~\ref{fig:undo_action_after}):
...
\subsection{Сценарий 7: Проверка на дубликаты}
...
\subsection{Сценарий 8: Сохранение и загрузка данных}
